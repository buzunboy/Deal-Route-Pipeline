...
### Admin-panel new-endpoint requests — ACR-10-Settings still needs a settings store (metrics layer now BUILT)
- **Severity**: low (the panel ships working placeholders for Settings; the rest of the ACR set is now BUILT — see below)
- **Area**: api / db / metrics
- **Location**: `src/adapters/http/review-api.ts` + `src/application/metrics/metrics.ts`; the panel's `docs/API_CHANGE_REQUESTS.md` (separate repo) has the exact contracts.
...
- **What's STILL deferred**: **ACR-10 Settings** (`GET /api/settings` + `PATCH /api/settings/:key`). The panel renders a zod-parsed placeholder and is **not blocked**.
- **Why deferred**: ACR-10 Settings needs a **settings store + a decision on which knobs are pipeline-owned vs deployment env** (most current config is env-driven, so a DB-backed settings table may be the wrong home — confirm with the owner first).
...
- **Fix-when**: build Settings only after the owner confirms what's pipeline-owned vs env. Add promote/extract audit rows when the audit screen needs the full action vocabulary.
- **Logged**: 2026-06-22 (updated 2026-06-22: ACR-5/7/8/10-published/10-sources/11/12 + the ACR-6/9/10-Metrics metrics layer all shipped; only ACR-10 Settings remains)
