...
### IdP /auth/{login,refresh} intermittently hang ~10s then 500 — RESOLVED 2026-06-26 (auth-path pool relief)
- **Was**: high (auth availability — a flapping IdP locks reviewers out of the panel), api / auth / db. On `dealroute-api-dev` (fra), `POST /auth/login` hung ~10s then 500'd; `POST /auth/refresh` 500'd fast; `GET /api/health` (DB-free, dispatched before the auth guard) + token'd reads stayed fast — an auth-surface-only flap (observed live 2026-06-26 from the Admin Panel; found via a 5-subsystem parallel investigation, 18 hypotheses → 10 survived adversarial verification). The **panel side was already mitigated** before this fix (Admin Panel commit `aa92368` on its `develop`: per-fetch client timeouts, 8s reads / 4s refresh) so a hung IdP no longer froze a reviewer's screen for ~130s — but the pipeline still flapped under any auth burst, so it's fixed here at the source. Panel side needs no further change.
- **Diagnosis (validated against the code + live logs, not trusted blindly)**: the single shared `pg.Pool({max:10, connectionTimeoutMillis:10_000})` (`postgres-db.ts` `connect`) is exhausted by the auth path's many INDEPENDENT checkouts per request — confirmed **9** on a successful login (`authenticate.ts`: getByEmail, getLoginState, getPasswordHashByEmail, recordLogin, then mintSession's permissionsForUser=2 + getPermVersion + roles.getById + refreshTokens.issue) and **7** on refresh (`refresh.ts`/`rotate`, the last a `db.transaction` pinning a slot). A saturated checkout throws pg-pool's **code-less** `Error('timeout exceeded when trying to connect')`; `isTransientDbError` keys off `err.code` (`db-resilience.ts`) so it is NOT retried → propagates → `tryMapAuthError` doesn't match it (`auth-error-map.ts`) → generic 500 via `serve.ts`'s `handler.catch`. **Binding-limit check (`fly logs -a dealroute-api-dev`)**: the buffer showed 34× `timeout exceeded when trying to connect` (client pool) AND 32× `Connection terminated unexpectedly` + 4× `Connection terminated due to connection timeout` (server-side: the **single `shared-cpu-1x:256MB` `dealroute-db` shared by prod+dev** dropping backends under pressure) — and **zero** `too_many_connections`/`53300`. So BOTH limits bind and reinforce each other; the diagnosed "fast 500s from a retried 53300" was NOT observed live.
- **Resolution**: connection-REDUCING + fail-fast fixes (correct under either binding limit; `DB_POOL_MAX` deliberately NOT raised — more client connections against a 256MB cluster already terminating backends makes the server side worse):
  1. **Coalesced the claim-minting reads** — new `Database.claimInputsForRole(roleId)` resolves (role permissions + role name + perm_version) in ONE checkout (Postgres: one `db.transaction`; in-memory: three composed reads). `mintSession`/`rotate` now key off the already-in-hand `user.role_id` (dropping the redundant `users.getById` the old `permissionsForUser(user.id)` did). Login 9→6 checkouts, refresh 7→4. Deny-by-default preserved (unknown role → empty). Covered by the shared auth-repositories contract suite (in-memory + Postgres, LSP).
  2. **Fail fast**: `DB_POOL_CONNECTION_TIMEOUT_MS` default 10_000 → **2_500** + new `DB_POOL_MIN` default **2** (warm connections so the first checkout after an idle spell doesn't pay a cold connect against the timeout).
  3. **503 not 500**: new `isPoolTimeoutError` (`db-resilience.ts`) recognises the code-less pool-timeout messages; `tryMapAuthError` maps it to **503 + Retry-After** (transient unavailability, not a request bug). Covered by a db-resilience unit test + an `auth-api` HTTP test (pool-timeout → 503).
- **Deferred (not in this change)**: the SERVER side — split the dev DB off the shared 256MB cluster (and/or move Postgres to managed). See the related open items "Dev DB shares the prod Postgres cluster" + "Production Postgres is self-managed (Fly)". This change relieves the client-pool pressure and degrades gracefully (503) under the server cap, but the shared cluster remains the root infra risk. If the flap recurs, re-confirm which limit binds: `fly logs -a dealroute-api-dev | grep -E "timeout exceeded when trying to connect|too_many_connections|connection evicted"`, and on the dev DB during a login burst `SELECT count(*) AS open, (SELECT setting::int FROM pg_settings WHERE name='max_connections') AS cap, sum((state='active')::int) AS active FROM pg_stat_activity;` — `too_many_connections` / a burst of `connection evicted` ⟶ server cap binds (split/size the DB, do NOT raise `DB_POOL_MAX`).
- **Gates**: `npm run check` (lint+typecheck+1113 unit) green; integration tier (real Postgres, incl. the Postgres contract run of `claimInputsForRole` + the auth-flow end-to-end) green; `api:lint`+`api:check` green (503 documented on `/auth/login`+`/auth/refresh`, Postman regenerated).
