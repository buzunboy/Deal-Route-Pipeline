...
- **Boundary validation after the LLM:** schema check, sanity (price ranges, currency = EUR for DE, valid dates), then **dedupe/canonicalize** on `service + provider + route_type + country + source origin` (split-by-source: the source URL's registrable domain is folded in, so each source's report of a route is its own record; the same source re-crawling the same route still collapses to one). Low confidence or failed rules → must-review (**never auto-publish**).
